...
def moma(wt_model, mutant_model, objective_sense='maximize', solver='gurobi',
    ...
    if minimize_norm:
        ...
        if not combined_model:
            ...
            ##                               eye(number_of_reactions,number_of_reactions)))
            ## if norm_type == 'linear':
            ##     raise Exception('Linear FBA not operational')
            ...
            #Add in the difference metabolites and their coefficients.
            #HERE:  This will be probably be slow with the objects
            ...
            [x.add_metabolites({objective_metabolite: x.objective_coefficient})
             for x in combined_model.reactions if x.objective_coefficient != 0]
            #Expand wt._S to include columns for mutant and difference reactions
            ## tmp_1 = s_hstack((wt_model._S,
            ##                   lil_matrix((wt_model._S.shape[0], (1 + reaction_coefficient)*number_of_reactions))))
            #Expand wt._S to include rows for mutant metabolites
            ...
            ## #Renaming of the Reactions and metabolites is no longer necessary.
